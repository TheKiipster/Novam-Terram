...
public class WorldGenRedSandSpike extends WorldGenerator
{
    public boolean generate(World worldIn, Random rand, BlockPos position)
    {
        ...
        if (worldIn.getBlockState(position).getBlock() != Blocks.SAND.getDefaultState().withProperty(BlockSand.VARIANT, BlockSand.EnumType.RED_SAND))
        {
            return false;
        }
        else
                ...
                for (int i1 = -l; i1 <= l; ++i1)
                {
                    ...
                    for (int j1 = -l; j1 <= l; ++j1)
                    {
                        ...
                        if ((i1 == 0 && j1 == 0 || f1 * f1 + f2 * f2 <= f * f) && (i1 != -l && i1 != l && j1 != -l && j1 != l || rand.nextFloat() <= 0.75F))
                        {
                            ...
                            {
                                this.setBlockAndNotifyAdequately(worldIn, position.add(i1, k, j1), Blocks.SAND.getDefaultState().withProperty(BlockSand.VARIANT, BlockSand.EnumType.RED_SAND));
                            }
            ...
            for (int l1 = -k1; l1 <= k1; ++l1)
            {
                for (int i2 = -k1; i2 <= k1; ++i2)
                {
                    ...
                    while (blockpos.getY() > 50)
                    {
                        ...
                        this.setBlockAndNotifyAdequately(worldIn, blockpos, Blocks.SAND.getDefaultState().withProperty(BlockSand.VARIANT, BlockSand.EnumType.RED_SAND));
