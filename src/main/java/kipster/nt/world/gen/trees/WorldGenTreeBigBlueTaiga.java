...
public class WorldGenTreeBigBlueTaiga extends WorldGenAbstractTree
{
    ...
    List<WorldGenTreeBigBlueTaiga.FoliageCoordinates> foliageCoords;
    ...
    void generateLeafNodeList()
    {
        ...
        this.foliageCoords = Lists.<WorldGenTreeBigBlueTaiga.FoliageCoordinates>newArrayList();
        this.foliageCoords.add(new WorldGenTreeBigBlueTaiga.FoliageCoordinates(this.basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (this.checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            this.foliageCoords.add(new WorldGenTreeBigBlueTaiga.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    void generateLeafNode(BlockPos pos)
    {
        ...
        {
            this.crosSection(pos.up(i), this.leafSize(i), BlockInit.SPRUCELEAVESBLUE.getDefaultState().withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false)));
        }
    }
    ...
    void generateLeaves()
    {
        for (WorldGenTreeBigBlueTaiga.FoliageCoordinates WorldGenTreeBigBlueTaiga$foliagecoordinates : this.foliageCoords)
        {
            this.generateLeafNode(WorldGenTreeBigBlueTaiga$foliagecoordinates);
        }
    }
    ...
    void generateTrunk()
    {
        ...
        IBlockState block = Blocks.LOG.getDefaultState().withProperty(BlockOldLog.VARIANT, BlockPlanks.EnumType.SPRUCE);
    ...
    private void limb(BlockPos blockpos, BlockPos blockpos1, IBlockState block) {
    ...
    void generateLeafNodeBases()
    {
        for (WorldGenTreeBigBlueTaiga.FoliageCoordinates WorldGenTreeBigBlueTaiga$foliagecoordinates : this.foliageCoords)
        {
            int i = WorldGenTreeBigBlueTaiga$foliagecoordinates.getBranchBase();
            BlockPos blockpos = new BlockPos(this.basePos.getX(), i, this.basePos.getZ());

            if (!blockpos.equals(WorldGenTreeBigBlueTaiga$foliagecoordinates) && this.leafNodeNeedsBase(i - this.basePos.getY()))
            {
                this.limb(blockpos, WorldGenTreeBigBlueTaiga$foliagecoordinates, Blocks.LOG.getDefaultState().withProperty(BlockOldLog.VARIANT, BlockPlanks.EnumType.SPRUCE));
            }
        }
    }

    private void limb(BlockPos blockpos, FoliageCoordinates worldGenTreeBigBlueTaiga$foliagecoordinates,
