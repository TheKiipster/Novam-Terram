...
public class WorldGenTreeBigMaple extends WorldGenAbstractTree
{
    private Random rand;
    private World world;
    ...
    void generateLeafNodeList()
    {
        ...
        this.foliageCoords = Lists.<WorldGenTreeBigMaple.FoliageCoordinates>newArrayList();
        this.foliageCoords.add(new WorldGenTreeBigMaple.FoliageCoordinates(this.basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (this.checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            this.foliageCoords.add(new WorldGenTreeBigMaple.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    void generateLeafNode(BlockPos pos)
    {
        ...
        {
            this.crosSection(pos.up(i), this.leafSize(i), BlockInit.MAPLELEAVES.getDefaultState().withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false)));
        }
    }
    ...
    void generateLeaves()
    {
        for (WorldGenTreeBigMaple.FoliageCoordinates WorldGenTreeBigMaple$foliagecoordinates : this.foliageCoords)
        {
            this.generateLeafNode(WorldGenTreeBigMaple$foliagecoordinates);
        }
    }
    ...
    void generateLeafNodeBases()
    {
        for (WorldGenTreeBigMaple.FoliageCoordinates WorldGenTreeBigMaple$foliagecoordinates : this.foliageCoords)
        {
            int i = WorldGenTreeBigMaple$foliagecoordinates.getBranchBase();
            BlockPos blockpos = new BlockPos(this.basePos.getX(), i, this.basePos.getZ());

            if (!blockpos.equals(WorldGenTreeBigMaple$foliagecoordinates) && this.leafNodeNeedsBase(i - this.basePos.getY()))
            {
                this.limb(blockpos, WorldGenTreeBigMaple$foliagecoordinates, Blocks.LOG);
            }
        }
    }
