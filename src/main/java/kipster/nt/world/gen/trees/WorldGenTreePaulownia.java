...
public class WorldGenTreePaulownia extends WorldGenAbstractTree
{
    ...
    List<WorldGenTreePaulownia.FoliageCoordinates> foliageCoords;

    public WorldGenTreePaulownia(boolean notify)
    {
        super(notify);
    }
    ...
    void generateLeafNodeList()
    {
        ...
        this.foliageCoords = Lists.<WorldGenTreePaulownia.FoliageCoordinates>newArrayList();
        this.foliageCoords.add(new WorldGenTreePaulownia.FoliageCoordinates(this.basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (this.checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            this.foliageCoords.add(new WorldGenTreePaulownia.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    void generateLeafNode(BlockPos pos)
    {
        ...
        {
            this.crosSection(pos.up(i), this.leafSize(i), BlockInit.PAULOWNIALEAVES.getDefaultState().withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false)));
        }
    }
    ...
    void generateLeaves()
    {
        for (WorldGenTreePaulownia.FoliageCoordinates WorldGenTreePaulownia$foliagecoordinates : this.foliageCoords)
        {
            this.generateLeafNode(WorldGenTreePaulownia$foliagecoordinates);
        }
    }
    ...
    void generateLeafNodeBases()
    {
        for (WorldGenTreePaulownia.FoliageCoordinates WorldGenTreePaulownia$foliagecoordinates : this.foliageCoords)
        {
            int i = WorldGenTreePaulownia$foliagecoordinates.getBranchBase();
            BlockPos blockpos = new BlockPos(this.basePos.getX(), i, this.basePos.getZ());

            if (!blockpos.equals(WorldGenTreePaulownia$foliagecoordinates) && this.leafNodeNeedsBase(i - this.basePos.getY()))
            {
                this.limb(blockpos, WorldGenTreePaulownia$foliagecoordinates, Blocks.LOG);
            }
        }
    }
