...
public class WorldGenTreePoplar2 extends WorldGenAbstractTree
{
    ...
    private static final IBlockState LEAF = Blocks.LEAVES.getDefaultState().withProperty(BlockOldLeaf.VARIANT, BlockPlanks.EnumType.BIRCH).withProperty(BlockLeaves.CHECK_DECAY, false);
    private final boolean useExtraRandomHeight;

    public WorldGenTreePoplar2(boolean notify, boolean useExtraRandomHeightIn)
    {
        super(notify);
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position)
    {
        ...
        if (position.getY() >= 1 && position.getY() + i <= 256)
        {
            for (int j = position.getY(); j <= position.getY() + i; ++j)
            {
                ...
                {
                    for (int i1 = position.getZ() + l; i1 <= position.getZ() + k && flag; ++i1)
                    {
            ...
            else
            {
                ...
                boolean isSoil = state.getBlock().canSustainPlant(state, worldIn, down, net.minecraft.util.EnumFacing.DOWN, (net.minecraft.block.BlockSapling)Blocks.SAPLING);
