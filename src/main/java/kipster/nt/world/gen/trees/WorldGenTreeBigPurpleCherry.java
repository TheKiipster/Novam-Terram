...
public class WorldGenTreeBigPurpleCherry extends WorldGenAbstractTree
{
    private Random rand;
    private World world;
    ...
    List<WorldGenTreeBigPurpleCherry.FoliageCoordinates> foliageCoords;

    public WorldGenTreeBigPurpleCherry(boolean notify)
    {
        super(notify);
    }
    ...
    void generateLeafNodeList()
    {
        ...
        this.foliageCoords = Lists.<WorldGenTreeBigPurpleCherry.FoliageCoordinates>newArrayList();
        this.foliageCoords.add(new WorldGenTreeBigPurpleCherry.FoliageCoordinates(this.basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (this.checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            this.foliageCoords.add(new WorldGenTreeBigPurpleCherry.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    void generateLeafNode(BlockPos pos)
    {
        ...
        {
            this.crosSection(pos.up(i), this.leafSize(i), BlockInit.CHERRYLEAVESPURPLE.getDefaultState().withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false)));
        }
    }
    ...
    void generateLeaves()
    {
        for (WorldGenTreeBigPurpleCherry.FoliageCoordinates WorldGenTreeBigJacaranda$foliagecoordinates : this.foliageCoords)
        {
            this.generateLeafNode(WorldGenTreeBigJacaranda$foliagecoordinates);
        }
    }
    ...
    void generateLeafNodeBases()
    {
        for (WorldGenTreeBigPurpleCherry.FoliageCoordinates WorldGenTreeBigJacaranda$foliagecoordinates : this.foliageCoords)
        {
            int i = WorldGenTreeBigJacaranda$foliagecoordinates.getBranchBase();
            BlockPos blockpos = new BlockPos(this.basePos.getX(), i, this.basePos.getZ());

            if (!blockpos.equals(WorldGenTreeBigJacaranda$foliagecoordinates) && this.leafNodeNeedsBase(i - this.basePos.getY()))
            {
                this.limb(blockpos, WorldGenTreeBigJacaranda$foliagecoordinates, Blocks.LOG);
            }
        }
    }
