...
public class WorldGenTreeBigRedOak extends WorldGenAbstractTree
{
    private Random rand;
    private World world;
    ...
    List<WorldGenTreeBigRedOak.FoliageCoordinates> foliageCoords;

    public WorldGenTreeBigRedOak(boolean notify)
    {
        super(notify);
    }
    ...
    void generateLeafNodeList()
    {
        ...
        this.foliageCoords = Lists.<WorldGenTreeBigRedOak.FoliageCoordinates>newArrayList();
        this.foliageCoords.add(new WorldGenTreeBigRedOak.FoliageCoordinates(this.basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (this.checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            this.foliageCoords.add(new WorldGenTreeBigRedOak.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    void generateLeafNode(BlockPos pos)
    {
        ...
        {
            this.crosSection(pos.up(i), this.leafSize(i), BlockInit.OAKLEAVESRED.getDefaultState().withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false)));
        }
    }
    ...
    void generateLeaves()
    {
        for (WorldGenTreeBigRedOak.FoliageCoordinates WorldGenTreeBigRedOak$foliagecoordinates : this.foliageCoords)
        {
            this.generateLeafNode(WorldGenTreeBigRedOak$foliagecoordinates);
        }
    }
    ...
    void generateLeafNodeBases()
    {
        for (WorldGenTreeBigRedOak.FoliageCoordinates WorldGenTreeBigRedOak$foliagecoordinates : this.foliageCoords)
        {
            int i = WorldGenTreeBigRedOak$foliagecoordinates.getBranchBase();
            BlockPos blockpos = new BlockPos(this.basePos.getX(), i, this.basePos.getZ());

            if (!blockpos.equals(WorldGenTreeBigRedOak$foliagecoordinates) && this.leafNodeNeedsBase(i - this.basePos.getY()))
            {
                this.limb(blockpos, WorldGenTreeBigRedOak$foliagecoordinates, Blocks.LOG);
            }
        }
    }
