...
public class WorldGenTreeBigOrchard extends WorldGenAbstractTree
{
    private Random rand;
    private World world;
    ...
    List<WorldGenTreeBigOrchard.FoliageCoordinates> foliageCoords;

    public WorldGenTreeBigOrchard(boolean notify)
    {
        super(notify);
    }
    ...
    void generateLeafNodeList()
    {
        ...
        this.foliageCoords = Lists.<WorldGenTreeBigOrchard.FoliageCoordinates>newArrayList();
        this.foliageCoords.add(new WorldGenTreeBigOrchard.FoliageCoordinates(this.basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (this.checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            this.foliageCoords.add(new WorldGenTreeBigOrchard.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    void generateLeafNode(BlockPos pos)
    {
        ...
        {
            this.crosSection(pos.up(i), this.leafSize(i), BlockInit.ORCHARDLEAVES.getDefaultState().withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false)));
        }
    }
    ...
    void generateLeaves()
    {
        for (WorldGenTreeBigOrchard.FoliageCoordinates WorldGenTreeBigJacaranda$foliagecoordinates : this.foliageCoords)
        {
