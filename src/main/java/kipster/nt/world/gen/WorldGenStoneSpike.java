...
public class WorldGenStoneSpike extends WorldGenerator
{
    public boolean generate(World worldIn, Random rand, BlockPos position)
    {
        ...
        if (worldIn.getBlockState(position).getBlock() != Blocks.STONE)
        {
            return false;
        }
        else
            ...
            for (int l1 = -k1; l1 <= k1; ++l1)
            {
                for (int i2 = -k1; i2 <= k1; ++i2)
                {
                    ...
                    while (blockpos.getY() > 50)
                    {
                        ...
                        if (!iblockstate1.getBlock().isAir(iblockstate1, worldIn, blockpos) && block1 != Blocks.DIRT && block1 != Blocks.SNOW && block1 != Blocks.ICE && block1 != Blocks.PACKED_ICE && block1 != Blocks.GRASS)
                        {
                            break;
                        }
